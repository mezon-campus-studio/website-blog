...
export class PostController {
    ...
    async createPost(req: Request, res: Response, next: NextFunction) {
        ...
        const post = await this.postService.createPost(req.body as CreatePostDto, userId, files.thumbnail?.[0], files.images);
    ...
    async updatePost(req: Request, res: Response, next: NextFunction) {
        ...
        const post = await this.postService.updatePost(req.body as UpdatePostDto, userId, postid, files.thumbnail?.[0], files.images);
    ...
    async saveDraft(req: Request, res: Response, next: NextFunction){
        const userId = (req as any).user.id;
        const postId = req.params.post_id as string;
        await this.postService.saveDraft(userId, postId);
        res.status(200).json({
    ...
    async publishPost(req: Request, res: Response, next: NextFunction){
        const userId = (req as any).user.id;
        const postId = req.params.post_id as string;
        await this.postService.publishPost(userId, postId);
        res.status(200).json({
