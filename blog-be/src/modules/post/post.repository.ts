...
export interface IPostRepository {
  createPost(data: Omit<CreatePostDto, 'images'> & {slug: string, thumbnailUrl: string | null,thumbnailPublicId: string | null,
    images: { url: string; publicId: string }[]}, userId: string, ): Promise<Post>;

  findBySlug(slug: string): Promise<Post | null>;

  updatePost(data: Omit<UpdatePostDto, 'images'> & {slug: string, thumbnailUrl: string | null,thumbnailPublicId: string | null,
  ...
  saveDraft(userId:string, postId:string): Promise<Post>;
